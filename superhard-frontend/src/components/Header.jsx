...
export default function Header({ user, setUser }) {
  ...
  return (
    <>
      {/* 🔹 Agregamos clases para hacerlo flotante */}
      <header className="fixed top-0 left-0 w-full bg-black text-white italic border-gray-300 z-50 shadow-md">
        ...
        <div className="flex flex-col md:flex-row py-[.6rem] px-[1rem] justify-between items-center gap-4 md:gap-0">
          ...
          <div className="flex text-lg gap-2">
            {!user ? (
              <Link to={"/login"} className="flex px-3 py-1 rounded gap-1 hover:cursor-pointer ">
            ...
            <Link to={"/shopping-cart"} className="flex px-3 py-1 rounded gap-1 hover:cursor pointer">
              <svg xmlns="http://www.w3.org/2000/svg" width={25} height={25} fill={"#FFFFFF"} viewBox="0 0 24 24">
        ...
        <nav className="flex py-[.3rem] bg-[#282828] justify-around gap-2">
          ...
          {categories.map((c) => (
            <button key={c} className="px-3 py-1 rounded hover:text-[#EEDA00] cursor-pointer">
              {c}
            </button>
          ))}
      ...
      {/* 🔹 Margen para que el contenido no quede debajo del header */}
